...
import { inboundApi, productApi } from '../api';
import { useWarehouse } from '../contexts/WarehouseContext';
import { useAuth } from '../contexts/AuthContext';
import DataTable from '../components/DataTable';
import Modal from '../components/Modal';
import { toast } from 'sonner';
import { Plus, ArrowDownLeft, CheckCircle, XCircle } from 'lucide-react';
...
export default function InboundPage() {
  ...
  const handleApprove = async (row) => {
    if (!confirm(`Duyệt phiếu ${row.inbound_code}?`)) return;
    try {
      await inboundApi.approve(row.id);
  ...
  const handleReject = async (row) => {
    if (!confirm(`Từ chối phiếu ${row.inbound_code}?`)) return;
    try {
      await inboundApi.reject(row.id);
  ...
  const columns = [
    { key: 'inbound_code', title: 'Mã phiếu', render: v => <code className="text-xs bg-teal-50 text-teal-700 px-1.5 py-0.5 rounded font-mono">{v}</code> },
    ...
    {
      key: 'actions', title: 'Thao tác', width: '160px',
      render: (_, row) => (
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="page-header">Phiếu Nhập kho</h1>
          <p className="text-sm text-gray-500 mt-1">Quản lý phiếu nhập hàng vào kho</p>
        </div>
        {hasRole('ADMIN', 'KHO') && (
      ...
      <DataTable columns={columns} data={data} loading={loading} emptyMessage="Chưa có phiếu nhập nào." />

      <Modal isOpen={modal.open} onClose={() => setModal({ open: false })} title="Tạo phiếu nhập kho" size="lg">
        <InboundForm
          products={products}
...
function InboundForm({ products, selectedWarehouseId, onSave, onCancel }) {
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true);
    try {
      await inboundApi.create({ warehouse_id: selectedWarehouseId, notes, items: validItems });
  ...
  return (
    <div className="space-y-4">
      <div className="p-3 bg-blue-50 rounded-lg text-sm text-blue-800">
        Đang nhập cho kho: <strong>{selectedWarehouseId || 'Chưa chọn'}</strong>. Sau khi duyệt, hệ thống sẽ tự động gợi ý kệ trống.
      </div>
      <div className="space-y-2">
        {items.map((item, i) => (
          <div key={i} className="flex gap-2 items-end">
            ...
            {items.length > 1 && (
              <button onClick={() => removeItem(i)} className="p-2 text-red-500 hover:bg-red-50 rounded"><XCircle className="w-4 h-4" /></button>
            )}
          </div>
        ))}
      </div>
